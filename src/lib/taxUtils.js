...
export function calculateTaxSplit(taxable, taxPercent, intraState) {
  ...
  if (intraState) {
    // For intra-state: CGST and SGST must sum exactly to total tax
    // Calculate CGST by rounding down half the tax to 2 decimals
    // Then SGST = tax - CGST to ensure they sum exactly to tax
    const cgst = +Math.floor(tax * 50) / 100  // Round down half tax to 2 decimals
    const sgst = +(tax - cgst).toFixed(2)  // Remainder ensures exact sum
