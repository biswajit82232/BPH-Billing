...
export default function SignaturePad({ value, onChange, label = 'Signature', width = 400, height = 200, showClearButton = false }) {
  ...
  const handlePadClick = (e) => {
    e.preventDefault()
    e.stopPropagation()
    // Prevent the click from causing form inputs to lose focus incorrectly
    if (isMobile) {
      setShowModal(true)
    }
  }
  ...
  const handlePadMouseDown = (e) => {
    // Prevent default to avoid triggering blur on other inputs
    e.stopPropagation()
    if (isMobile) {
      e.preventDefault()
    }
      ...
      <div className="space-y-2 w-full">
        ...
        <div className="flex items-center justify-between">
          {value ? (
            ...
            <span className="text-xs text-gray-400">No signature captured</span>
          )}
          {showClearButton && (
            <button
              type="button"
