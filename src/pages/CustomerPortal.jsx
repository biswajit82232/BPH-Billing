...
export default function CustomerPortal() {
  ...
  const customerInvoices = useMemo(() => {
    ...
    return invoices.filter((invoice) => {
      // Check multiple possible phone locations and formats
      const invoicePhone1 = (invoice.phone || '').toString().replace(/\D/g, '')
      const invoicePhone2 = (invoice.customer?.phone || '').toString().replace(/\D/g, '')
      
      // Match if either phone matches (handle last 10 digits for international numbers)
      const match1 = invoicePhone1.length >= 10 && invoicePhone1.slice(-10) === phone
      const match2 = invoicePhone2.length >= 10 && invoicePhone2.slice(-10) === phone
      
      return match1 || match2 || invoicePhone1 === phone || invoicePhone2 === phone
    }).sort((a, b) => {
