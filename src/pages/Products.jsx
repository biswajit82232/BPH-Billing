...
export default function Products() {
  ...
  const handleDistributorSubmit = async (event) => {
    ...
    try {
      const saved = await upsertDistributor({
        ...newDistributor,
        name: newDistributor.name.trim(),
      })
      setShowDistributorModal(false)
      setNewDistributor({ name: '', phone: '', contact: '', note: '' })
      setForm((prev) => ({ ...prev, distributorId: saved.id }))
      toast.success(`Distributor "${saved.name}" added`)
    } catch (error) {
      console.error('Distributor save error', error)
  ...
  const handlePurchase = async (event) => {
    ...
    try {
      ...
      if (isNewProduct) {
        ...
        // Create new product
        let distributorId = ''
        const supplierName = purchaseRow.supplier?.trim()
        if (supplierName) {
          const existingDistributor = distributors.find(
            (dist) => dist.name.toLowerCase() === supplierName.toLowerCase(),
          )
        ...
        <section id="item-form" className="glass-panel p-4 md:p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Used to calculate amount owed to suppliers.</p>
            </div>
            <div>
        ...
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="text-gray-400 hover:text-gray-600" onClick={() => setShowDistributorModal(false)}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>
            <form className="space-y-3" onSubmit={handleDistributorSubmit}>
              <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewDistributor((prev) => ({ ...prev, contact: e.target.value }))}
                    className="w-full"
              ...
              <div>
                ...
                <textarea
                  value={newDistributor.note}
                  onChange={(e) => setNewDistributor((prev) => ({ ...prev, note: e.target.value }))}
                  className="w-full"
                  rows="2"
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t">
                <button type="button" className="btn-secondary" onClick={() => setShowDistributorModal(false)}>
                  Cancel
                </button>
