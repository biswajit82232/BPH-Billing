...
import { InvoiceListSkeleton } from '../components/LoadingSkeleton'

const ITEMS_PER_PAGE = 50

const getSortValue = (invoice, field) => {
  switch (field) {
    case 'date': {
      const parsed = safeParseDate(invoice.date)
      return parsed ? parsed.getTime() : 0
    }
    case 'dueDate': {
      const parsed = safeParseDate(invoice.dueDate)
      return parsed ? parsed.getTime() : 0
    }
    case 'invoiceNo':
      return invoice.invoiceNo || ''
    case 'customer':
      return (invoice.customerName || '').toLowerCase()
    case 'total':
...
export default function InvoiceList() {
  const { invoices, markInvoiceStatus, deleteInvoice, refreshData, loading } = useData()
  ...
  const selectedSet = useMemo(() => new Set(selectedInvoices), [selectedInvoices])
  ...
  const handleSelectInvoice = useCallback((invoiceId, checked) => {
  ...
  const searchQuery = useMemo(() => debouncedSearch.toLowerCase().trim(), [debouncedSearch])
  
  // Pre-parse dates once
  const fromDateParsed = useMemo(() => (from ? safeParseDate(from) : null), [from])
  const toDateParsed = useMemo(() => (to ? safeParseDate(to) : null), [to])

  const { filtered, paginated, totalPages, totalAmount } = useMemo(() => {
    // Fast path: no filters applied
    if (status === 'all' && !searchQuery && !fromDateParsed && !toDateParsed) {
      ...
      return { filtered: sorted, paginated, totalPages, totalAmount }
    }
    ...
    let filtered = invoices.filter((invoice) => {
      ...
      // Search filter (early exit if no match)
      if (searchQuery) {
        const invoiceNoLower = invoice.invoiceNo.toLowerCase()
        const customerNameLower = invoice.customerName.toLowerCase()
        const phone = (invoice.phone || invoice.customer?.phone || '').toString()
        
        if (!invoiceNoLower.includes(searchQuery) && 
            !customerNameLower.includes(searchQuery) && 
            !phone.includes(searchQuery)) {
          return false
        }
      }
      
      // Date filters (only parse if dates are provided)
      if (fromDateParsed || toDateParsed) {
      const invoiceDate = safeParseDate(invoice.date)
        if (!invoiceDate) return false
        
        if (fromDateParsed && safeCompareDates(invoiceDate, fromDateParsed) < 0) return false
        if (toDateParsed && safeCompareDates(invoiceDate, toDateParsed) > 0) return false
      }
  ...
  }, [invoices, searchQuery, fromDateParsed, toDateParsed, status, currentPage, sortField, sortOrder])
            ...
            <table className="w-full">
              ...
              <tbody>
                {paginated.map((invoice) => (
                  <tr key={invoice.id} className={selectedSet.has(invoice.id) ? 'bg-blue-50' : ''}>
                    <td>
                      <input
                        type="checkbox"
